  ...
  create: function (context) {
    ...
    function visitFunctionExpression(node) {
      ...
      if (!declaredDependenciesNode) {
        ...
        if (reactiveHookName === 'useMemo' || reactiveHookName === 'useCallback') {
          context.report({
            ...
            message: 'React Hook ' + reactiveHookName + ' doesn\'t serve any purpose ' + 'without a dependency array. To enable ' + 'this optimization, pass an array of values used by the ' + ('inner function as the second argument to ' + reactiveHookName + '.')
          });
        }
        return;
      }
      ...
      if (problemCount === 0) {
        ...
        bareFunctions.forEach(function (_ref3) {
          var fn = _ref3.fn,
              suggestUseCallback = _ref3.suggestUseCallback;
      ...
      if (unnecessaryDependencies.size > 0) {
        ...
        if (badRef !== null) {
          ...
          if (!scope.set.has(dep)) {
            extraWarning = ' Outer scope values like \'' + dep + '\' aren\'t valid dependencies ' + 'because their mutation doesn\'t re-render the component.';
          }
        }
      }
      ...
      if (!extraWarning && missingDependencies.has('props')) {
        ...
        if (isPropsOnlyUsedInMembers) {
          extraWarning = ' However, the preferred fix is to destructure the \'props\' ' + ('object outside of the ' + reactiveHookName + ' call and ') + 'refer to specific props directly by their names.';
        }
      }
      ...
      if (!extraWarning && missingDependencies.size > 0) {
        ...
        if (missingCallbackDep !== null) {
          extraWarning = ' If specifying \'' + missingCallbackDep + '\'' + ' makes the dependencies change too often, ' + 'find the parent component that defines it ' + 'and wrap that definition in useCallback.';
        }
      }
        ...
        if (setStateRecommendation !== null) {
          switch (setStateRecommendation.form) {
            case 'reducer':
              ...
              break;
            case 'inlineReducer':
              extraWarning = ' You can also replace useState with an inline useReducer ' + ('if \'' + setStateRecommendation.setter + '\' needs the ') + ('current value of \'' + setStateRecommendation.missingDep + '\'.');
              break;
            case 'updater':
              extraWarning = ' You can also write \'' + setStateRecommendation.setter + '(' + setStateRecommendation.missingDep.substring(0, 1) + ' => ...)\' if you only use \'' + setStateRecommendation.missingDep + '\'' + (' for the \'' + setStateRecommendation.setter + '\' call.');
              break;
            default:
...
function collectRecommendations(_ref4) {
  var dependencies = _ref4.dependencies,
      declaredDependencies = _ref4.declaredDependencies,
      optionalDependencies = _ref4.optionalDependencies,
      externalDependencies = _ref4.externalDependencies,
      isEffect = _ref4.isEffect;
...
function scanForDeclaredBareFunctions(_ref7) {
  var declaredDependencies = _ref7.declaredDependencies,
      declaredDependenciesNode = _ref7.declaredDependenciesNode,
      componentScope = _ref7.componentScope,
      scope = _ref7.scope;

  var bareFunctions = declaredDependencies.map(function (_ref8) {
    var key = _ref8.key;
